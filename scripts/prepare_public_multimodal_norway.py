#!/usr/bin/env python3
"""
Prepare public multi-modal demo assets for one Norway-first maritime demo pack.

This script augments an existing gravity+bathymetry demo pack with:
- scalar magnetic total field from NOAA WMMHR2025
- scalar magnetic anomaly as WMMHR2025 - WMM2025
- static surface current field from HYCOM GLBy0.08 latest point queries
- a tide-correction config reference

The runtime estimator stack is unchanged. This script only prepares the local
data products and writes an augmented demo-pack manifest.
"""
...
def _decimal_year_from_iso8601(iso_time: str) -> float:
    dt = datetime.fromisoformat(str(iso_time).replace("Z", "+00:00")).astimezone(timezone.utc)
...
def _run_wmm_file(
    ...
    *,
    ...
    subprocess.run(
        ...
        # NOAA's geomag tools expect their coefficient file to live in cwd.
        cwd=work_dir,
        check=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
...
def _fill_invalid_current_rows(
    ...
    *,
    ...
    lat_lookup = {float(lat): i for i, lat in enumerate(np.asarray(lat_axis_deg, dtype=np.float64))}
    lon_lookup = {float(lon): j for j, lon in enumerate(np.asarray(lon_axis_deg, dtype=np.float64))}
...
def _derive_output_paths(
    *,
    ...
    if pack_stem.endswith("_demo_pack"):
        output_pack_name = f"{pack_stem[:-10]}_priority9_demo_pack.json"
    else:
        output_pack_name = f"{pack_stem}_priority9.json"
    return {
...
def _augment_demo_pack(
    ...
    *,
    ...
    return replace(
        ...
        tide_config_path=str(tide_config_path.resolve()),
        notes=notes,
        metadata=metadata,
    )
...
def _prepare_assets(
    ...
    *,
    ...
    anomaly_rows = {
        key: float(wmmhr_rows[key] - wmm_rows[key])
        for key in wmmhr_rows
    }
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="",
        help="Optional output path for the augmented demo pack. Defaults to a sibling *_priority9_demo_pack.json.",
    )
    parser.add_argument(
        "--vendor-cache-dir",
        default=str(DEFAULT_VENDOR_CACHE_DIR),
        help="Ignored local cache directory for NOAA WMM/WMMHR downloads and builds.",
    )
    parser.add_argument(
        "--tide-config",
        default=str(DEFAULT_TIDE_CONFIG),
        help="Tide/datum correction config referenced by the augmented demo pack.",
    )
    parser.add_argument(
        "--snapshot-time-iso",
        default="",
        help="Optional fixed ISO-8601 UTC time for HYCOM queries, e.g. 2026-04-14T03:00:00Z. Defaults to the latest HYCOM sample time discovered at the region center.",
    )
    parser.add_argument(
        "--magnetic-decimal-year",
        type=float,
        default=None,
        help="Optional decimal year override for WMM/WMMHR evaluation. Defaults to the snapshot time converted to decimal year.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    _prepare_assets(
        ...
        snapshot_time_iso=(
            None if not str(args.snapshot_time_iso).strip() else str(args.snapshot_time_iso).strip()
        ),
        magnetic_decimal_year=(
            None if args.magnetic_decimal_year is None else float(args.magnetic_decimal_year)
        ),
