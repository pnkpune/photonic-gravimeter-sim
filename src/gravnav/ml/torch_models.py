    ...
    for _ in range(int(train_spec.epochs)):
        ...
        for start in range(0, num_examples, batch_size):
            ...
            logits = net.score_candidates(
                ...
                candidate_offsets_ned_m=candidate_offsets[batch],
                analytic_log_emission=analytic[batch],
            )
            posterior = torch.softmax(logits, dim=1)
            expected_offsets = torch.sum(
                posterior.unsqueeze(-1) * candidate_offsets[batch, :, :2],
                dim=1,
            )
            ce_loss = F.cross_entropy(logits, labels[batch])
            offset_loss = F.smooth_l1_loss(expected_offsets, truth_offsets[batch, :2])
            loss = ce_loss + float(train_spec.offset_loss_weight) * offset_loss
